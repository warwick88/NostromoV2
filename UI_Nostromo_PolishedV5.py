...
image_viewer_column2 = [
    ...
    [sg.Button('Enter2', bind_return_key=True), sg.Button("Exit")],
     ]
# ----- Full layout -----
layout = [
...
while True:  # Event Loop
    ...
    if event == "Engineer Game":
        ...
        #image_viewer_column
        #window[f'-layout{image_viewer_column2}-'.update(visible=False)
        #window[image_viewer_column2].update(visible=True)
        #window[f'-COL3-{image_viewer_column2}-'].update(visible=True)
        #window.Refresh()
        # Here we will build the game that alters the right column in the GUI for the game to launch
        # Early reading suggests you need to use columns and either hide them or make them invisible,
        # or add or remove them to the existing structure, make true or false fields for differnt aspects
        # thinking about it now, probably implies we make current session INVISIBLE and make the engineer window
        # visible.
        #layout = [
         #   [sg.Column(file_list_column, background_color='black'),
          #   sg.VSeperator(),
           #  sg.Column(image_viewer_column2, background_color='black')],
        #]
       #  below I think you have to refresh the window to update the GUI to actually show changes. Still not sure
